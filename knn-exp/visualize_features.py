...
FEATURIZED_DATA_DIR = os.path.join(DATA_DIR, "featurized-data-baseline/") # Or the original baseline
...
ELEMENT_PROPS_LIST = [
    'atomic_mass', 'atomic_radius', 'atomic_volume', # Or melting_point if you changed it
...
def load_base_featurized_data(feature_dir):
    ...
    try:
        ...
        print(f"Error: Base featurized data not found in {feature_dir}. File: {e.filename}", file=sys.stderr)
...
def plot_property_distributions_base(features, element_map, props_list, plots_dir):
    ...
    if features.shape[1] != expected_dims:
        print(f"Error: Feature dimension mismatch. Expected {expected_dims}, got {features.shape[1]}.", file=sys.stderr)
        return

    prop_features_data = features[:, num_elements_in_map:] # Extract only property columns
    ...
    for i in range(num_props):
        ...
        if len(data_col) > 0:
             sns.histplot(data_col, kde=True, ax=ax, color='mediumseagreen', edgecolor='black', linewidth=0.5)
...
def plot_tsne_projection_base(features, targets, plots_dir, n_samples_tsne=2000, perplexity_tsne=30):
    ...
    scaler = StandardScaler() # For base k-NN, QuantileTransformer might have been used in train.
    ...
    except Exception as e: print(f"Error scaling Base data for t-SNE: {e}", file=sys.stderr); return

    print(f"  Running t-SNE on Base features (perplexity={current_perplexity}, samples={scaled_features.shape[0]})...")
    ...
    scatter = plt.scatter(tsne_results[:, 0], tsne_results[:, 1],
                        c=num_elements_list, cmap='cividis', alpha=0.65, s=15, edgecolor='none') # Different cmap
...
if __name__ == "__main__":
    ...
    features_arr, elem_map, target_formulas = load_base_featurized_data(FEATURIZED_DATA_DIR)

    if features_arr is not None and elem_map is not None and target_formulas is not None:
        ...
        plot_property_distributions_base(features_arr, elem_map, ELEMENT_PROPS_LIST, PLOTS_DIR)
        
        plot_tsne_projection_base(features_arr, target_formulas, PLOTS_DIR, n_samples_tsne=2000)

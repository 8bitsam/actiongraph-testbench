...
def load_data_from_directory(dir_path):
    ...
    for root, _, files in os.walk(dir_path):
        ...
        for filename in json_files:
            ...
            except Exception: error_count += 1 # Simplified error handling
    end_time = time.time()
    print(f"Finished scanning. Loaded {loaded_count} entries, skipped {error_count} files.")
    print(f"Data loading took {end_time - start_time:.2f} seconds.")
    if not all_data:
        print(f"Error: No valid JSON data loaded from directory: {dir_path}", file=sys.stderr)
        return None
    return all_data
...
def get_element_map(data):
    ...
    for i, entry in enumerate(data):
        ...
        try:
            ...
            if comp.num_atoms < 1: continue
            for el in comp.elements: all_elements.add(el.symbol)
            parsed_count += 1
        except Exception: parse_error_count +=1
    if parse_error_count > 0: print(f"Warning: Could not parse target formula for {parse_error_count} entries during element map creation.")
    try:
        common_elements = {el.symbol for el in Element}
        all_elements.update(common_elements)
    except Exception as e: print(f"Warning: Could not retrieve all Pymatgen elements: {e}", file=sys.stderr)
...
def featurize_target(formula, element_map, element_props_list):
    ...
    try:
        ...
        for element_symbol, amount in el_amt_dict.items():
            if element_symbol in element_map: comp_vector[element_map[element_symbol]] = amount / total_atoms
            ...
            for el_symbol in elements_in_comp_symbols:
                 ...
                 try:
                     ...
                     if prop_value is not None and isinstance(prop_value, (int, float)):
                         ...
                         prop_available = True
                 except Exception: pass # Catch all errors during property lookup
            if prop_available and total_amount_for_prop > 0: prop_vector[i] = weighted_sum / total_amount_for_prop
...
def get_recipe_hash(entry):
    """Creates a unique hash for a recipe based on target, precursors, and operations."""
    try:
        ...
        return hashlib.md5(str(entry.get('_source_file', os.urandom(16))).encode('utf-8')).hexdigest()
...
def run_featurize():
    ...
    for entry in raw_data:
        ...
        if processed_for_dedup % 10000 == 0: print(f"  Checked {processed_for_dedup}/{len(raw_data)} for duplicates...")
    ...
    if not deduplicated_data:
        print("Error: No unique data entries remaining after deduplication.", file=sys.stderr)
        return False
    ...
    for i, entry in enumerate(deduplicated_data):
        ...
        valid_entry = (isinstance(target_data, dict) and target_formula and
                       isinstance(target_formula, str) and precursors is not None and
                       isinstance(precursors, list) and operations is not None and
                       isinstance(operations, list))

        if valid_entry:
            feature_vector = featurize_target(target_formula, element_map, ELEMENT_PROPS)
        ...
        if (i + 1) % 20000 == 0: print(f"  Featurized {i+1}/{len(deduplicated_data)} unique entries...")
    ...
    try:
        ...
        with open(os.path.join(FEATURIZED_DATA_DIR, 'recipes.json'), 'w') as f: json.dump(recipes, f)
        with open(os.path.join(FEATURIZED_DATA_DIR, 'targets.json'), 'w') as f: json.dump(targets, f)
        with open(os.path.join(FEATURIZED_DATA_DIR, 'original_indices.json'), 'w') as f: json.dump(original_indices, f) # These are identifiers from unique entries
        with open(os.path.join(FEATURIZED_DATA_DIR, 'element_map.json'), 'w') as f: json.dump(element_map, f)
    except Exception as e:

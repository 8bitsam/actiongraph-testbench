...
def compare_recipes_detailed(pred_recipe, true_recipe, is_rf=False):
    ...
    results["operations_length_match"] = (len(pred_ops) == len(true_ops)) if not is_rf else False
...
def run_evaluate(model_type='knn'):
    ...
    try:
        ...
        with open(os.path.join(FEATURIZED_DATA_DIR, 'recipes.json'), 'r') as f: all_recipes = json.load(f)
        with open(os.path.join(FEATURIZED_DATA_DIR, 'targets.json'), 'r') as f: all_targets = json.load(f)
        with open(os.path.join(FEATURIZED_DATA_DIR, 'original_indices.json'), 'r') as f: all_original_indices = json.load(f)
        ...
        print(f"Error: Featurized data not found in {FEATURIZED_DATA_DIR}. Run 'featurize'.", file=sys.stderr)
        return False
    ...
    try:
        ...
        if model_type == 'knn':
            ...
            if os.path.exists(os.path.join(MODEL_DIR, 'rf_prec_model.joblib')):
                 model_artifacts['rf_prec_model'] = joblib.load(os.path.join(MODEL_DIR, 'rf_prec_model.joblib'))
                 model_artifacts['prec_binarizer'] = joblib.load(os.path.join(MODEL_DIR, 'prec_binarizer.joblib'))
            if os.path.exists(os.path.join(MODEL_DIR, 'rf_op_type_model.joblib')):
                 model_artifacts['rf_op_type_model'] = joblib.load(os.path.join(MODEL_DIR, 'rf_op_type_model.joblib'))
                 model_artifacts['op_type_binarizer'] = joblib.load(os.path.join(MODEL_DIR, 'op_type_binarizer.joblib'))
        else:
            raise ValueError(f"Unknown model_type '{model_type}' during artifact loading.")

    except FileNotFoundError:
        print(f"Error: Model artifacts not found in {MODEL_DIR}. Run 'train --model_type {model_type}'.", file=sys.stderr)
        return False
    except Exception as e:
    ...
    test_original_ids = [all_original_indices[i] for i in test_orig_indices_all] # Renamed for clarity

    print(f"Test set size: {len(X_test)}")
    if len(X_test) == 0:
        print("Warning: Test set has size 0 after split. Evaluation cannot proceed.", file=sys.stderr)
        return True
    ...
    if model_type == 'knn':
        ...
        train_indices = model_artifacts['train_indices'] # Indices of training samples within all_recipes
        y_train_recipes = [all_recipes[i] for i in train_indices] # Get actual training recipes
        ...
        for i in range(len(X_test_scaled)):
            ...
            if pred_prec_multi is not None and prec_binarizer:
                ...
                if i < len(pred_prec_multi):
                    try:
                         ...
                         pred_prec_tuple = prec_binarizer.inverse_transform(pred_prec_multi[i].reshape(1,-1))
                         if pred_prec_tuple: pred_prec_set = set(pred_prec_tuple[0])
                    except ValueError as e:
                         print(f"Warning: Error transforming precursor prediction for sample {i}: {e}", file=sys.stderr)
                else: print(f"Warning: Missing precursor prediction for sample {i}.", file=sys.stderr)
            ...
            if pred_op_type_multi is not None and op_type_binarizer:
                 if i < len(pred_op_type_multi):
                      try:
                           pred_op_type_tuple = op_type_binarizer.inverse_transform(pred_op_type_multi[i].reshape(1,-1))
                           if pred_op_type_tuple: pred_op_type_set = set(pred_op_type_tuple[0])
                      except ValueError as e:
                           print(f"Warning: Error transforming op_type prediction for sample {i}: {e}", file=sys.stderr)
                 else: print(f"Warning: Missing op_type prediction for sample {i}.", file=sys.stderr)
            ...
            predictions.append({
                ...
                'operations': [{'type': op} for op in pred_op_type_set] # Structure for comparison func
            })

    else:
        print(f"Error: Unknown model_type '{model_type}' for prediction.", file=sys.stderr)
        return False
    ...
    if evaluation_results:
        ...
        for key in metric_keys:
            ...
            if valid_values:
                 if isinstance(valid_values[0], bool): avg_results[f"avg_{key}"] = np.mean([float(v) for v in valid_values])
                 elif isinstance(valid_values[0], (int, float)): avg_results[f"avg_{key}"] = np.mean(valid_values)
                 else: avg_results[f"avg_{key}"] = "N/A (non-numeric)"
            else: avg_results[f"avg_{key}"] = "N/A (no valid data)"
    ...
    print(f"{'avg_operations_exact_match':<30}: {avg_results.get('avg_operations_exact_match', 'N/A'):.4f} {exact_match_note}")
    print(f"{'avg_operations_length_match':<30}: {avg_results.get('avg_operations_length_match', 'N/A'):.4f} {length_match_note}")
    ...
    if model_type == 'knn':
         ...
         print("P/R/F1 scores evaluate the accuracy of the retrieved sets from that neighbor.")

    # --- Show Example ---
    print("\n--- Example Prediction ---")
    if len(predictions) > 0:
        example_idx_in_test = 0
        print(f"Test Entry Original Identifier: {test_original_ids[example_idx_in_test]}")

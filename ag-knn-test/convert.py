...
import sys
import time
# import hashlib # Not using MD5 for this version
from tqdm import tqdm

try:
    from actiongraph import ActionGraph # Your ActionGraph class
except ImportError:
    print("Error: actiongraph.py not found or cannot be imported.", file=sys.stderr)
    sys.exit(1)
...
FILTERED_AG_OUTPUT_DIR = os.path.join(DATA_DIR, "filtered-ag-data/") # Output

def load_mp_json_files(dir_path, desc="Loading MP JSONs"):
    """Loads all .json files from a directory, adding a source file identifier."""
        ...
        try:
            with open(filepath, 'r', encoding='utf-8') as f:
                ...
                if isinstance(data_entry, dict):
                    data_entry['_source_file'] = os.path.basename(filepath) # Store basename
                    all_data.append(data_entry)
        except Exception:
            pass # Silently skip files that fail to load or are not dicts
...
def get_canonical_ag_json_string_for_hash(ag_serial_data: dict) -> str:
    ...
    try:
        data_to_hash = json.loads(json.dumps(ag_serial_data)) # Deep copy
        ...
        edge_key = 'links' if 'links' in data_to_hash else ('edges' if 'edges' in data_to_hash else None)
        if edge_key and edge_key in data_to_hash and isinstance(data_to_hash[edge_key], list):
            data_to_hash[edge_key] = sorted(data_to_hash[edge_key], key=lambda x: (str(x.get('source','')), str(x.get('target',''))))
        ...
        return str(ag_serial_data.get('_source_file', os.urandom(16))) # Fallback
...
def run_conversion_to_ag():
    ...
    for mp_entry in tqdm(raw_mp_entries, desc="MP to AG Conversion"):
        ...
        if action_graph_instance:
            try:
                ...
                # Temporarily store original filename for output naming
    ...
    print(f"Successfully serialized {len(serialized_ags_with_source_info)} ActionGraphs.")
    ...
    if not serialized_ags_with_source_info: return False

    print("\nDeduplicating serialized ActionGraphs by content...")
    unique_ag_map_for_dedup = {} # hash_value -> ag_data_with_source_info
    ...
    for ag_data_item in tqdm(serialized_ags_with_source_info, desc="Deduplicating AGs"):
        ...
        data_for_hashing_step.pop('_original_mp_filename', None) # Exclude temp key from hash
        
        canonical_ag_str = get_canonical_ag_json_string_for_hash(data_for_hashing_step)
        content_hash_val = hash(canonical_ag_str) # Using Python's built-in hash
    ...
    print(f"Removed {ag_duplicate_removed_count} duplicate AGs. Unique AGs: {len(final_unique_serialized_ags)}")
    if not final_unique_serialized_ags: return False
    ...
    for ag_data_to_save_final in tqdm(final_unique_serialized_ags, desc="Saving AGs"):
        original_mp_filename = ag_data_to_save_final.pop('_original_mp_filename', f"filtered_ag_{final_saved_count}.json")
        # Use original MP filename (without .json) + _ag.json for new AG filename
        ...
        # Basic collision handling if multiple MP files map to same AG output name (unlikely after AG dedup)
        counter = 0
        temp_final_path = output_ag_filepath
        while os.path.exists(temp_final_path):
            counter += 1
            temp_final_path = os.path.join(FILTERED_AG_OUTPUT_DIR, f"{base_mp_name}_{counter}_ag.json")
    ...
    print(f"Saved {final_saved_count} unique ActionGraph JSON files to {FILTERED_AG_OUTPUT_DIR}.")

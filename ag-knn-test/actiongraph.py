...
class ActionGraph(nx.DiGraph):
    ...
    def _parse_amount(self, amount_data: Any) -> float:
        if isinstance(amount_data, (int, float)): return float(amount_data)
        elif isinstance(amount_data, str):
            try: return float(amount_data)
            ...
            if 'values' in amount_data and isinstance(amount_data['values'], list) and amount_data['values']:
            ...
            if 'min_value' in amount_data and 'max_value' in amount_data:
                try:
                    min_val, max_val = float(amount_data['min_value']), float(amount_data['max_value'])
    ...
    def _add_node_safe(self, node_id: str, **attrs):
        if node_id in self.nodes: self.nodes[node_id].update(attrs)
        else: self.add_node(node_id, **attrs)

    def add_input(self, chemical_data: Dict, node_id: Optional[str] = None) -> str:
        if not (isinstance(chemical_data, dict) and \
                'material_formula' in chemical_data and \
                chemical_data['material_formula'] and \
                isinstance(chemical_data['material_formula'], str)):
            raise ValueError("Invalid chemical_data for input node.")
        if node_id is None: node_id = f"chem_in_{len(self.input_nodes)}"
        try: composition_obj = Composition(chemical_data['material_formula'])
        except Exception as e: raise ValueError(f"Invalid Pymatgen formula: {e}")
        amount, elements_data = 1.0, {}
        if 'composition' in chemical_data and isinstance(chemical_data['composition'], list) and chemical_data['composition']:
        ...
        features = {'type': 'chemical', 'formula': chemical_data['material_formula'],
                    'composition': composition_obj, 'amount': amount, 'elements': elements_data}
        self._add_node_safe(node_id, **features)
        if node_id not in self.input_nodes: self.input_nodes.append(node_id)
        return node_id

    def _parse_single_condition_value_entry(self, entry: Any) -> List[float]:
        values = []
        if isinstance(entry, (int, float)): values.append(float(entry))
    ...
    def _parse_conditions(self, cond_input: Dict) -> Dict:
        parsed_cond = {'temperature_K': [], 'time_s': [], 'atmosphere': [],
        ...
        for temp_entry in cond_input.get('heating_temperature', []):
            parsed_cond['temperature_K'].extend(self._parse_single_condition_value_entry(temp_entry))
        ...
        for time_entry in cond_input.get('heating_time', []):
            parsed_cond['time_s'].extend(self._parse_single_condition_value_entry(time_entry))
        if parsed_cond['time_s']:
             parsed_cond['time_s'] = sorted(list(set(parsed_cond['time_s'])))
        atm_raw = cond_input.get('heating_atmosphere', [])
        if isinstance(atm_raw, list): parsed_cond['atmosphere'] = [str(a) for a in atm_raw if a is not None]
        elif atm_raw is not None: parsed_cond['atmosphere'] = [str(atm_raw)]
        ...
        for key, value in cond_input.items():
            if key not in ['heating_temperature', 'heating_time', 'heating_atmosphere', 'mixing_device', 'mixing_media']:
                try: json.dumps(value); parsed_cond['other_conditions'][key] = value
                except TypeError: parsed_cond['other_conditions'][key] = str(value)
        return parsed_cond

    def add_operation(self, op_data: Dict, source_nodes: List[str],
                      node_id: Optional[str] = None) -> str:
        if not isinstance(op_data, dict): raise ValueError("Invalid op_data.")
        ...
        self._add_node_safe(node_id, **features)
        if node_id not in self.operation_nodes: self.operation_nodes.append(node_id)
        for src_node_id in source_nodes:
            if src_node_id in self.nodes:
                self.add_edge(src_node_id, node_id)
        return node_id

    def add_output(self, target_data: Dict, source_operation_node_id: str,
                     node_id: Optional[str] = None) -> str:
        if not (isinstance(target_data, dict) and \
                'material_formula' in target_data and \
                target_data['material_formula'] and \
                isinstance(target_data['material_formula'], str)):
            raise ValueError("Invalid target_data for output node.")
        if source_operation_node_id not in self.nodes or self.nodes[source_operation_node_id].get('type') != 'operation':
            raise ValueError(f"Source '{source_operation_node_id}' must be an existing operation node.")
        if node_id is None: node_id = f"chem_out_{len(self.output_nodes)}"
        try: composition_obj = Composition(target_data['material_formula'])
        except Exception as e: raise ValueError(f"Invalid Pymatgen formula for output: {e}")
        amount, elements_data = 1.0, {}
        if 'composition' in target_data and isinstance(target_data['composition'], list) and target_data['composition']:
        ...
        features = {'type': 'chemical', 'formula': target_data['material_formula'],
                    'composition': composition_obj, 'amount': amount, 'elements': elements_data}
        self._add_node_safe(node_id, **features)
        if node_id not in self.output_nodes: self.output_nodes.append(node_id)
    ...
    def display(self, figsize=(10,7), prog="dot"):
        ...
        if self.number_of_nodes() == 0:
            plt.text(0.5, 0.5, "Empty Graph", ha="center", va="center", fontsize=ACTIONGRAPH_FONT_SIZE_MEDIUM)
            plt.axis('off'); plt.show(); return
        try: pos = nx.nx_agraph.graphviz_layout(self, prog=prog)
        except ImportError:
            pos = nx.spring_layout(self, seed=42, k=0.9 / (self.number_of_nodes()**0.5) if self.number_of_nodes() > 0 else 0.8, iterations=50)
        
        chem_color, op_color = '#1f77b4', '#ff7f0e'
        node_colors_list = [chem_color if self.nodes[node_id].get('type') == 'chemical' 
                            else (op_color if self.nodes[node_id].get('type') == 'operation' else '#9467bd')
                            for node_id in self.nodes()]

        nx.draw_networkx_nodes(self, pos, node_color=node_colors_list, 
                               node_size=ACTIONGRAPH_NODE_SIZE, edgecolors='black', linewidths=1)
        nx.draw_networkx_edges(self, pos, edge_color=ACTIONGRAPH_EDGE_COLOR,
                               arrows=True, arrowsize=ACTIONGRAPH_ARROW_SIZE, 
                               arrowstyle='-|>', width=ACTIONGRAPH_EDGE_WIDTH,
        ...
        for node_id, attrs in self.nodes(data=True):
            ...
            if attrs.get('type') == 'chemical':
                ...
                try: pretty_formula = Composition(formula_str).get_reduced_formula_and_factor(iupac_ordering=True)[0]
                except: pretty_formula = formula_str
                labels[node_id] = f"{short_id}\n{pretty_formula}"
            elif attrs.get('type') == 'operation':
                op_type_obj = attrs.get('op_type')
                op_type_str = op_type_obj.name if isinstance(op_type_obj, OperationTypeEnum) else str(op_type_obj)
                if "Operation" in op_type_str: op_type_str = op_type_str.replace("Operation", "Op.")
                if "Synthesis" in op_type_str: op_type_str = op_type_str.replace("Synthesis", "Synth.")
                labels[node_id] = f"{short_id}\n{op_type_str}"
            else: labels[node_id] = f"{short_id}\n(Unk)"
        nx.draw_networkx_labels(self, pos, labels=labels, font_size=ACTIONGRAPH_FONT_SIZE_SMALL)
        legend_elements = [mpatches.Patch(color=chem_color, label='Chemical Node'),
        ...
        plt.legend(handles=legend_elements, loc='upper center', bbox_to_anchor=(0.5, 1.02),
                   ncol=2, fancybox=True, frameon=False, fontsize=ACTIONGRAPH_FONT_SIZE_SMALL)
        plt.axis('off'); plt.tight_layout(pad=0.1); plt.show()
    ...
    def serialize(self) -> Dict:
        ...
        # Edge attributes are not explicitly removed, but new ones won't have 'edge_type'
    ...
    def deserialize(cls, data: Dict) -> 'ActionGraph':
        ...
        if edge_key is None: raise KeyError("Edge key not found in serialized data.")
        graph_data_for_nx = {'nodes': data.get('nodes', []), edge_key: data.get(edge_key, []),
                             'directed': data.get('directed', True), 'multigraph': data.get('multigraph', False),
        ...
        try:
            ...
            for link_item in links_data_list:
                ...
                source_id, target_id = node_id_map.get(source_orig, str(source_orig)), node_id_map.get(target_orig, str(target_orig))
                edge_attrs = {k: v for k, v in link_item.items() if k not in ['source', 'target', 'key', 'edge_type']}
        ...
        for node_id, attrs in ag.nodes(data=True):
            if attrs.get('type') == 'chemical' and 'composition' in attrs and isinstance(attrs['composition'], str):
                try: attrs['composition'] = Composition(attrs['composition'])
                except: 
                    try: attrs['composition'] = Composition.from_dict(json.loads(attrs['composition']))
                    except: pass
            elif attrs.get('type') == 'chemical' and 'formula' in attrs and 'composition' not in attrs :
                 try: attrs['composition'] = Composition(attrs['formula'])
                 except: pass
        ...
        if not any([ag.input_nodes, ag.operation_nodes, ag.output_nodes]):
            ...
            for node_id, attrs_node in ag.nodes(data=True):
                ...
                if node_type == 'chemical':
                    is_output = False
                    if ag.out_degree(node_id) == 0: # Potential output
                        # An output chemical must be preceded by an operation node
                    ...
                    elif ag.in_degree(node_id) == 0 and not is_output: # Must not also be an output
    ...
    def from_mp_synthesis(cls, mp_data: dict) -> Union["ActionGraph", None]:
        ...
        if not all(key in mp_data and mp_data[key] for key in required_keys): return None
        try:
            if not (isinstance(mp_data["precursors"], list) and \
                    all(isinstance(p, dict) and p.get('material_formula') for p in mp_data["precursors"])):
                return None
            ag = cls(input_chemicals=mp_data["precursors"])
            current_source_node_ids = ag.input_nodes[:]
            if not current_source_node_ids and mp_data["precursors"]: return None

            if not mp_data.get("operations") or not isinstance(mp_data["operations"], list) or \
               not all(isinstance(op, dict) and 'type' in op for op in mp_data["operations"]):
                if ag.input_nodes and mp_data.get("target"): 
                    generic_op_id = ag.add_operation(
                        ...
                        source_nodes=ag.input_nodes, node_id="op_assumed_0")
                    current_source_node_ids = [generic_op_id]
                else: return None
            else:
                for op_idx, operation_data in enumerate(mp_data["operations"]):
                    if not current_source_node_ids and op_idx > 0 : return None
                    op_id = ag.add_operation(op_data=operation_data, 
                                             source_nodes=current_source_node_ids if current_source_node_ids else [],
                                             node_id=f"op_{op_idx}")
                    current_source_node_ids = [op_id]
            
            if not current_source_node_ids: return None 
            last_operation_node_id = current_source_node_ids[0]
            if not (isinstance(mp_data["target"], dict) and mp_data["target"].get("material_formula")):
                return None
            ag.add_output(target_data=mp_data["target"], source_operation_node_id=last_operation_node_id)
            return ag

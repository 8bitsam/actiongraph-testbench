...
try:
    ...
    print("Ensure featurize_ag.py, train_ag.py, evaluate_ag.py are accessible.", file=sys.stderr)
    sys.exit(1)

def main():
    parser = argparse.ArgumentParser(description='Inorganic Synthesis Prediction Pipeline (ActionGraph Version)')
    ...
    parser.add_argument('--steps', nargs='+',
                        ...
                        default=['all_ag'], help='Steps to run in the AG pipeline')
    ...
    if model_type != 'knn':
        print(f"Warning: This pipeline version is optimized for 'knn' with ActionGraph features. "
              f"Specified model_type '{model_type}' will proceed as 'knn'.", file=sys.stderr)
        model_type = 'knn'
    ...
    if 'all_ag' in steps_to_run or 'featurize_ag' in steps_to_run:
        ...
        if not run_featurize_ag(): # run_featurize_ag is specific, doesn't need model_type
            print("Featurize AG step failed. Aborting pipeline.", file=sys.stderr)
            success = False
        else:
            print(f"Featurize AG step finished in {time.time() - step_start_time:.2f} seconds.")
    ...
    if success and ('all_ag' in steps_to_run or 'train_ag' in steps_to_run):
        ...
        if not run_train_ag(model_type=model_type): # Pass model_type (knn)
            ...
            success = False
             ...
             print(f"Train AG step finished in {time.time() - step_start_time:.2f} seconds.")
    ...
    if success and ('all_ag' in steps_to_run or 'evaluate_ag' in steps_to_run):
        ...
        if not run_evaluate_ag(model_type=model_type): # Pass model_type (knn)
            ...
            success = False
             ...
             print(f"Evaluate AG step finished in {time.time() - step_start_time:.2f} seconds.")

    end_pipeline_time = time.time()
    print(f"\n--- ActionGraph Pipeline Finished (Success: {success}) ---")
    print(f"Total AG pipeline execution time: {end_pipeline_time - start_pipeline_time:.2f} seconds.")

...
ELEMENT_PROPS_LIST = ['atomic_mass', 'atomic_radius', 'melting_point', 'X', 'electron_affinity', 'ionization_energy']
...
def load_ag_featurized_data(feature_dir_ag):
    ...
    try:
        ...
        with open(os.path.join(feature_dir_ag, 'targets_ag_output_formula.json'), 'r') as f:
        ...
        if os.path.exists(pca_model_path):
            ...
            print(f"Warning: PCA model 'adj_pca.joblib' not found. PCA related plots might be affected or pca_n_components inferred.")
            # pca_n_components remains 0 if not found

        print(f"Loaded {features_ag.shape[0]} AG samples with {features_ag.shape[1]} features.")
        if pca_n_components > 0:
            print(f"  PCA components from loaded model: {pca_n_components}")
        elif os.path.exists(pca_model_path):
             print(f"  PCA model 'adj_pca.joblib' loaded, but n_components_ is 0 or not set as expected.")
        return features_ag, element_map_ag, targets_ag, pca_n_components
    except FileNotFoundError as e:
        print(f"Error: Featurized AG data or PCA model not found in {feature_dir_ag}. File: {e.filename}", file=sys.stderr)
...
def plot_target_property_distributions_ag(features_ag, element_map, props_list, plots_dir):
    ...
    if target_prop_features_end_idx > features_ag.shape[1]:
        print(f"Error: Index for target properties ({target_prop_features_end_idx}) exceeds feature dimension ({features_ag.shape[1]}).", file=sys.stderr)
        return

    target_prop_features = features_ag[:, target_prop_features_start_idx:target_prop_features_end_idx]
    ...
    for i in range(num_props):
        ...
        if len(data_col) > 0:
             sns.histplot(data_col, kde=True, ax=ax, color='skyblue', edgecolor='black', linewidth=0.5)
...
def plot_pca_explained_variance(pca_model_path, plots_dir):
    ...
    if not os.path.exists(pca_model_path):
        print(f"  PCA model file not found at {pca_model_path}. Skipping explained variance plot.")
        return
    try:
        pca = joblib.load(pca_model_path)
        if not hasattr(pca, 'explained_variance_ratio_'):
            print("  Loaded PCA model does not have 'explained_variance_ratio_'. Skipping plot.")
            return
        ...
        plt.step(range(1, len(explained_variance_ratio) + 1), np.cumsum(explained_variance_ratio), where='mid',
                 label='Cumulative', color='firebrick', linewidth=2)
        ...
        tick_step = max(1, num_components_to_show_ticks // 10 if num_components_to_show_ticks > 0 else 1) 
        if num_components_to_show_ticks < 10 and num_components_to_show_ticks > 0: tick_step = 1
        
        if num_components_to_show_ticks > 0:
            plt.xticks(ticks=np.arange(1, num_components_to_show_ticks + 1, step=tick_step))
...
def plot_tsne_projection_ag(features_ag, targets_ag, plots_dir, n_samples_tsne=2000, perplexity_tsne=30):
    print(f"\nPerforming t-SNE projection on AG features (on max {n_samples_tsne} samples)...")
    os.makedirs(plots_dir, exist_ok=True)

    if features_ag.shape[0] == 0: return
    actual_n_samples = min(n_samples_tsne, features_ag.shape[0])
    if features_ag.shape[0] > actual_n_samples:
        indices = np.random.choice(features_ag.shape[0], actual_n_samples, replace=False)
    ...
    if len(features_subset) <= current_perplexity :
        ...
        if current_perplexity < 5:
            print(f"  Skipping t-SNE: Samples ({len(features_subset)}) too few for perplexity {current_perplexity}.", file=sys.stderr)
            return
    ...
    except Exception as e: print(f"Error scaling for t-SNE: {e}", file=sys.stderr); return

    print(f"  Running t-SNE (perplexity={current_perplexity}, samples={scaled_features.shape[0]})...")
    start_tsne_time = time.time()
    tsne = TSNE(n_components=2, random_state=42, perplexity=current_perplexity, n_iter=300, init='pca', learning_rate='auto')
    try:
        tsne_results = tsne.fit_transform(scaled_features)
    except Exception as e_tsne: print(f"Error during t-SNE: {e_tsne}", file=sys.stderr); return
    print(f"  t-SNE finished in {time.time() - start_tsne_time:.2f} seconds.")

    plt.figure(figsize=(7, 6)) 
    num_elements_list = []
    for formula in targets_subset:
        try: num_elements_list.append(len(Composition(formula).elements))
        except: num_elements_list.append(0) 

    scatter = plt.scatter(tsne_results[:, 0], tsne_results[:, 1],
                        c=num_elements_list, cmap='viridis', alpha=0.65, s=15, edgecolor='none')
    ...
    plt.yticks([])
    try:
        cbar = plt.colorbar(scatter, label='# Elements in Target Formula', fraction=0.046, pad=0.04)
...
if __name__ == "__main__":
    print("--- Running ActionGraph Feature Visualization (PCA version - No PCA Component Distributions) ---")
    os.makedirs(PLOTS_DIR_AG, exist_ok=True)

    features_ag_arr, elem_map_ag, target_formulas_ag, pca_n_components_loaded = load_ag_featurized_data(FEATURIZED_DATA_DIR_AG)

    if features_ag_arr is not None and elem_map_ag is not None and target_formulas_ag is not None:
        ...
        if pca_n_components_loaded is None: pca_n_components_loaded = 0

        # Final check of feature dimensions
        if features_ag_arr.shape[1] != num_total_target_chem_features + pca_n_components_loaded:
            print(f"CRITICAL ERROR: Loaded feature dimension ({features_ag_arr.shape[1]}) does not match expected sum of chemical features ({num_total_target_chem_features}) and PCA components ({pca_n_components_loaded}).")
            print("  Please check feature generation script (featurize.py) and constants here.")
            sys.exit(1)

        plot_target_property_distributions_ag(features_ag_arr, elem_map_ag, ELEMENT_PROPS_LIST, PLOTS_DIR_AG)
        ...
        plot_pca_explained_variance(pca_model_filepath, PLOTS_DIR_AG) # This will still run if adj_pca.joblib exists
        
        plot_tsne_projection_ag(features_ag_arr, target_formulas_ag, PLOTS_DIR_AG, n_samples_tsne=2000)

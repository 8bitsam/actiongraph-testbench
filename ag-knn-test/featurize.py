...
import joblib
import argparse # Added for standalone script execution options

try:
    from actiongraph import ActionGraph
except ImportError:
    print("Error: actiongraph.py not found or ActionGraph class cannot be imported.", file=sys.stderr)
    sys.exit(1)
...
FEATURIZED_DATA_DIR_AG = os.path.join(DATA_DIR, "featurized-data-actiongraph/") # Default output
ELEMENT_PROPS = ['atomic_mass', 'atomic_radius', 'melting_point', 'X', 'electron_affinity', 'ionization_energy']
MAX_NODES = 31 # Updated
PCA_COMPONENTS_DEFAULT = 20 # Default value if not overridden

warnings.filterwarnings("ignore", category=UserWarning, module="pymatgen")
warnings.filterwarnings("ignore", category=RuntimeWarning)

# Helper Functions (load_mp_and_ag_data_pairs, get_element_map_from_ag_data, 
# get_recipe_hash, featurize_target_formula, featurize_adj_matrix - keep these as they were in the last correct version)

def load_mp_and_ag_data_pairs(ag_dir_path, mp_dir_path):
    if not os.path.isdir(ag_dir_path):
        print(f"Error: AG data path is not a valid directory: {ag_dir_path}", file=sys.stderr)
        return []
    if not os.path.isdir(mp_dir_path):
        print(f"Error: MP data path is not a valid directory: {mp_dir_path}", file=sys.stderr)
        return []
    ...
    for root, _, files in os.walk(ag_dir_path):
        ...
        for ag_filename in ag_json_files:
            ...
            if mp_data_dict is None: continue
    ...
    if mp_file_missing_count > 0: print(f"Skipped {mp_file_missing_count} pairs due to missing MP files.")
    if mp_load_errors > 0 : print(f"Skipped {mp_load_errors} pairs due to MP file loading errors.")
    if ag_load_errors > 0 : print(f"Skipped {ag_load_errors} pairs due to AG file loading errors.")
...
def get_element_map_from_ag_data(payloads_for_map_creation):
    ...
    print(f"Creating element map from {len(payloads_for_map_creation)} unique AG structures for map.")
    for i, (ag_data_dict, _, _, ag_filename_map) in enumerate(payloads_for_map_creation):
        try:
    ...
    if ag_deserialize_errors_map > 0:
        print(f"Warning: Failed to deserialize {ag_deserialize_errors_map} AGs during element map creation.")
    if formula_parse_errors_map > 0:
        print(f"Warning: Could not parse {formula_parse_errors_map} chemical formulas during element map creation.")
    ...
    try:
        ...
        print(f"Warning: Could not retrieve all Pymatgen elements: {e}", file=sys.stderr)

    sorted_elements = sorted(list(all_elements))
    if not sorted_elements:
         print("Error: No valid elements found from ActionGraphs for element map.", file=sys.stderr)
         return None
    return {element: i for i, element in enumerate(sorted_elements)}

def get_recipe_hash(entry): # For MP-style recipe deduplication
    try:
        ...
        for op in operations:
            if isinstance(op, dict):
                try:
                    op_strings_list.append(json.dumps(op, sort_keys=True, separators=(',',':')))
                except TypeError: 
...
def featurize_target_formula(formula, element_map, element_props_list):
    ...
    if not element_map: return None
    try:
        comp = Composition(formula)
            ...
            for el_symbol in elements_in_comp:
                ...
                try:
                    ...
                    if callable(attr_or_method):
                        try: prop_value = attr_or_method()
                        except Exception: prop_value = None 
                    else: prop_value = attr_or_method
        ...
        return np.nan_to_num(np.concatenate((comp_vector, prop_vector)), nan=0.0, posinf=0.0, neginf=0.0)
    except Exception:
        return None

def featurize_adj_matrix(ag_object, max_nodes):
    if not isinstance(ag_object, nx.DiGraph): return np.zeros(max_nodes * max_nodes, dtype=np.float32)
...
# --- Main Featurization Logic ---
def run_featurize_ag(pca_n_components_arg=None): # Modified to accept argument
    
    # Determine PCA components: use argument if provided, else global default
    if pca_n_components_arg is not None:
        current_pca_components = pca_n_components_arg
        print(f"--- Running ActionGraph Featurization with PCA (Components: {current_pca_components} from arg) ---")
    else:
        current_pca_components = PCA_COMPONENTS_DEFAULT
        print(f"--- Running ActionGraph Featurization with PCA (Components: {current_pca_components} from default) ---")
    ...
    for _, ag_d, ag_fn in loaded_pairs:
        try:
            ag_data_str_canonical = json.dumps(ag_d, sort_keys=True, separators=(',',':'))
        except TypeError: 
    ...
    for i, payload_tuple in enumerate(loaded_pairs):
        ...
        try:
            ...
            if ag is None: skipped_count_total += 1; continue
            if not ag.output_nodes: skipped_count_total += 1; continue
            output_node_id = ag.output_nodes[0]
            if output_node_id not in ag.nodes or not ag.nodes[output_node_id].get('formula'):
                skipped_count_total += 1; continue
            formula = ag.nodes[output_node_id]['formula']
            
            chem_features = featurize_target_formula(formula, element_map, ELEMENT_PROPS)
            if chem_features is None: skipped_count_total += 1; continue

            adj_features_flat = featurize_adj_matrix(ag, MAX_NODES)
            if adj_features_flat is None: skipped_count_total += 1; continue # Should be zeros now
            
            chemical_features_list.append(chem_features)
            adjacency_matrices_list.append(adj_features_flat)
            recipes_for_eval_list.append({"precursors": mp_data.get('precursors', []), "operations": mp_data.get('operations', [])})
    ...
    if adj_array.ndim == 1 and adj_array.size > 0 :
        if len(adjacency_matrices_list) == 1: adj_array = adj_array.reshape(1, -1)
        else: print(f"CRITICAL ERROR: adj_array 1D. Shape: {adj_array.shape}. Exiting."); return False
    if adj_array.shape[0] == 0: print("CRITICAL ERROR: adj_array empty. Exiting."); return False
    if adj_array.ndim != 2: print(f"CRITICAL ERROR: adj_array not 2D. Shape: {adj_array.shape}. Exiting."); return False

    adj_scaler = StandardScaler()
    n_pca_effective = min(current_pca_components, adj_array.shape[0], adj_array.shape[1])
    ...
    if n_pca_effective > 0:
        if n_pca_effective < current_pca_components:
            print(f"Warning: Requested PCA components ({current_pca_components}) reduced to {n_pca_effective}.")
        ...
        print(f"Warning: PCA not performed (effective components is 0). PCA features will be empty.")

    print(f"Adjacency matrices PCA reduced. Shape: {reduced_adj.shape}")
    
    final_features_list = []
    if len(chemical_features_list) != reduced_adj.shape[0]:
        print(f"CRITICAL ERROR: Mismatch btw chemical features ({len(chemical_features_list)}) & PCA'd adj ({reduced_adj.shape[0]}). Exiting.")
        return False
    ...
    np.save(os.path.join(FEATURIZED_DATA_DIR_AG, 'features_ag.npy'), final_feature_array)
    if pca is not None: joblib.dump(pca, os.path.join(FEATURIZED_DATA_DIR_AG, 'adj_pca.joblib'))
    joblib.dump(adj_scaler, os.path.join(FEATURIZED_DATA_DIR_AG, 'adj_scaler.joblib'))
    with open(os.path.join(FEATURIZED_DATA_DIR_AG, 'element_map_ag.json'), 'w') as f: json.dump(element_map, f)    
    with open(os.path.join(FEATURIZED_DATA_DIR_AG, 'recipes_mp_style_for_ag.json'), 'w') as f: json.dump(recipes_for_eval_list, f)
    with open(os.path.join(FEATURIZED_DATA_DIR_AG, 'targets_ag_output_formula.json'), 'w') as f: json.dump(targets_list, f)
    with open(os.path.join(FEATURIZED_DATA_DIR_AG, 'original_ag_ids.json'), 'w') as f: json.dump(original_ag_ids_list, f)
    
    actual_saved_pca_components = reduced_adj.shape[1]
    print(f"Final feature dimension: {final_feature_array.shape[1]} (Chem: {chemical_features_list[0].shape[0]} + Adj_PCA: {actual_saved_pca_components})")
    print(f"--- ActionGraph Featurization with PCA (Components: {current_pca_components}) Completed ---")
    return True

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Featurize ActionGraph data with PCA.')
    parser.add_argument('--pca_components', type=int, default=PCA_COMPONENTS_DEFAULT,
    ...
    if not run_featurize_ag(pca_n_components_arg=args.pca_components):
        sys.exit(1) # Exit with error if featurization fails

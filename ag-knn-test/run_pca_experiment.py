...
RESULTS_DIR = os.path.abspath(os.path.join(current_dir, "..", "Data", "pca_experiment_results_pub/"))
...
def plot_f1_vs_pca_from_data(pca_values, precursor_f1_scores, operation_f1_scores, output_dir):
    """
    Generates and saves the F1 vs. PCA components plot.
    """
    ...
    valid_indices = ~np.isnan(precursor_f1_scores_arr) & ~np.isnan(operation_f1_scores_arr)
    ...
    if len(plotted_pca_values) > 0:
        plt.plot(plotted_pca_values, plotted_prec_f1, marker='o', linestyle='-', color='royalblue', 
                 label='Precursors F1 Score', markersize=6, linewidth=1.5)
        plt.plot(plotted_pca_values, plotted_ops_f1, marker='s', linestyle='--', color='orangered', 
                 label='Operations Type F1 Score', markersize=6, linewidth=1.5)
        ...
        if len(plotted_pca_values) > 1:
            ...
            if len(plotted_pca_values) <=10 : tick_step = 1
        ...
        if len(all_plotted_f1_scores) > 0:
            ...
            if padding < 0.01 and max_f1 - min_f1 < 0.02 : padding = 0.01 # Ensure minimal padding if range is tiny

            plot_bottom = max(0, min_f1 - padding)
            plot_top = min(1, max_f1 + padding)   

            if plot_bottom >= plot_top : # Handle cases where all values are identical or very close to 0 or 1
...
def plot_results_from_json(json_filepath, output_dir_for_plot):
    ...
    try:
        ...
        print(f"Error reading or parsing JSON file {json_filepath}: {e}", file=sys.stderr)
        return False
    ...
    if pca_components is None or precursor_f1 is None or operation_f1 is None:
        print("Error: JSON file is missing one or more required keys ('pca_components', 'precursor_f1_scores', 'operation_f1_scores').", file=sys.stderr)
        return False
        
    plot_f1_vs_pca_from_data(pca_components, precursor_f1, operation_f1, output_dir_for_plot)
    return True
...
def run_full_experiment():
    ...
    for n_components_loop in pca_values_loop:
        ...
        if not run_featurize_ag(pca_n_components_arg=n_components_loop):
            print(f"Featurization failed for {n_components_loop} components. Appending NaN and skipping.")
        ...
        if not run_train_ag(model_type='knn'):
            print(f"Training failed for {n_components_loop} components. Appending NaN and skipping.")
            precursor_f1_scores_collected.append(np.nan)
            operation_f1_scores_collected.append(np.nan)
            continue
        ...
        if not eval_metrics or not isinstance(eval_metrics, dict):
            print(f"Evaluation failed or returned invalid metrics for {n_components_loop} components. Appending NaN.")
            precursor_f1_scores_collected.append(np.nan)
            operation_f1_scores_collected.append(np.nan)
            continue
        
        current_prec_f1 = eval_metrics.get("avg_precursors_f1", np.nan) # Use NaN if key missing
        current_ops_f1 = eval_metrics.get("avg_op_type_f1", np.nan)   # Use NaN if key missing

        print(f"Evaluation took {time.time() - start_time_step:.2f}s")
        print(f"  Results for {n_components_loop} PCA: Precursor F1 = {current_prec_f1:.4f}, Operations F1 = {current_ops_f1:.4f}")
    ...
    plot_f1_vs_pca_from_data(pca_values_loop, precursor_f1_scores_collected, operation_f1_scores_collected, RESULTS_DIR)

    print("\n--- Full PCA Components Experiment Finished ---")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run PCA component experiment or plot results from JSON.")
    parser.add_argument('--plot_from_json', type=str, default=None,
    ...
    if args.plot_from_json:
        ...
        if not os.path.isabs(json_file_to_plot) and not os.path.exists(json_file_to_plot):
        ...
        if not plot_results_from_json(json_file_to_plot, RESULTS_DIR):
            print(f"Could not plot from {json_file_to_plot}. Try running the experiment first.")
            sys.exit(1)
        ...
        if os.path.exists(default_json_path):
            print(f"No specific action requested. Attempting to plot from default results file: {default_json_path}")
            ...
            print(f"Or use --plot_from_json <path_to_your_results.json> to plot existing results.")
            parser.print_help()
